...
class BasicModel(nn.Module):
    ...
    def encode(self, src_sents_var, src_sents_len, src_type, q_onehot_project=None):
        """
        ...
        """
        src_token_embed = self.gen_x_batch_with_type(src_sents_var, src_type)
    ...
    def gen_x_batch_with_type(self, q, src_type):
        B = len(q)
        val_embs = []
        ...
        for i, one_q in enumerate(q):
            ...
            # print(src_type[i])
            type_val = self.one_hot_type_embed(self.new_tensor(src_type[i])).cpu().detach().numpy().tolist()
            v_val = [(q_val[i] + type_val[i]) / 2 for i in range(len(q_val))]
            
            val_embs.append(v_val)
            val_len[i] = len(q_val)
        max_len = max(val_len)
